...
import com.prithwiraj.ibc.activities.SelectFieldActivity;
...
public class SelectFieldAdapter extends ArrayAdapter {
    ...
    public SelectFieldAdapter(@NonNull SelectFieldActivity context, @LayoutRes int resource, JSONArray optionsJsonArrayObj, String dataKey, String selection) {
        ...
        if (this.isJsonArray == true) {
            if (this.optionsJsonArray != null && this.dataKey != null && this.selection != null&&this.seats!=null) {
                for (int i = 0; i < optionsJsonArray.length(); i++) {

                    try {
                        JSONObject obj = optionsJsonArray.getJSONObject(i);
                        String valueStr = obj.optString(this.dataKey+this.seats);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (this.displayArray.length() > position) {
            if (this.isJsonArray == true) {
                ...
                if (optionJsonObject != null) {
                    ...
                    displayValue = optionJsonObject.optString("name");
                    displayValue1=optionJsonObject.optString("seats");
        ...
        if(dataKey.matches("name")){
            holder.itemName.setText(displayValue+" (Seats-"+displayValue1+")");
        }
        else{
